    ...
    if(len(sys.argv)<4):
        ...
        for j in range(len(num_feats)): #Para cada Num_feats
            ...
            for i in range(iter): #Realizamos i iteraciones
                ...
                n,r,cm= main.trainEvaluation(all,lr,max_epoch,int(num_feats[j])) #n -> Total accuracy, r -> interval radius, cm -> confusion matrix
                if(bn < n):
                    bn = n
            ...
            media_nf = total/i
            print('*'*50)
            print('Para',num_feats[j],'Num feats, y despues de',iter,'inicializaciones, tenemos:')
            media_error = 100 - media_nf
